...
**La existencia de una fila en `usuario` ES el consentimiento**, y su llave
es `identidad_hash`: la huella del BSUID que manda Meta (ADR-0023,
migración `010`). Se llamaba `telefono_hash` hasta el 17/09/2026, y el
nombre viejo mentiría, porque el teléfono ya no se guarda de ninguna
forma. Cada espacio de identificadores lleva su propia etiqueta de dominio
en el HMAC —`bsuid:`, como ya se hacía con `wamid:`—, así que dos huellas
de espacios distintos no pueden coincidir aunque compartan pepper y
columna.
